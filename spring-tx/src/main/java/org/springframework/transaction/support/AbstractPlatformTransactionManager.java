...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	private TransactionStatus handleExistingTransaction(
		...
		// PROPAGATION_NOT_SUPPORTED挂起当前事务，创建空事务
		...
		// PROPAGATION_REQUIRES_NEW挂起当前事务，创建新事物
	...
	protected final SuspendedResourcesHolder suspend(@Nullable Object transaction) throws TransactionException {
		if (TransactionSynchronizationManager.isSynchronizationActive()) {
			...
			try {
				...
				if (transaction != null) {
					// 把transaction中的connectionHolder设为null
					// 解除TransactionSynchronizationManager中dataSource和connectionHolder的绑定
					// suspendedResources就是解除绑定的connectionHolder
					suspendedResources = doSuspend(transaction);
				}
				// 从TransactionSynchronizationManager中得到当前事务的一些属性
				...
				// 用SuspendedResourcesHolder保存上面的这些属性
	...
	private void cleanupAfterCompletion(DefaultTransactionStatus status) {
		status.setCompleted();
		if (status.isNewSynchronization()) {
			// 如果不在外部事务当中，清空TransactionSynchronizationManager
			TransactionSynchronizationManager.clear();
		}
		if (status.isNewTransaction()) {
			// 重置连接，如果连接属于当前事务就释放连接
